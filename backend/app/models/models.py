...
class WayfairOrder(Base):
    """Wayfair 订单"""
    __tablename__ = "wayfair_orders"

    id = Column(Integer, primary_key=True, index=True)
    po_number = Column(String(100), unique=True, index=True, nullable=False)
    order_type = Column(String(50), default="dropship")
    ...
    ship_by = Column(DateTime, nullable=True)
    raw_data = Column(JSON, default=dict)
    created_at = Column(DateTime, default=lambda: datetime.now(ZoneInfo("Asia/Shanghai")))
    updated_at = Column(DateTime, default=lambda: datetime.now(ZoneInfo("Asia/Shanghai")), onupdate=lambda ctx: datetime.now(ZoneInfo("Asia/Shanghai")))

    items = relationship("WayfairOrderItem", back_populates="order")


class WayfairOrderItem(Base):
    """Wayfair 订单明细"""
    __tablename__ = "wayfair_order_items"

    id = Column(Integer, primary_key=True, index=True)
    order_id = Column(Integer, ForeignKey("wayfair_orders.id"), nullable=False)
    order = relationship("WayfairOrder", back_populates="items")
    sku = Column(String(100), index=True)
    supplier_part_number = Column(String(100), index=True)
    name = Column(String(500))
    quantity = Column(Integer, default=1)
    price = Column(Float, default=0)
    raw_data = Column(JSON, default=dict)
...
class WayfairInventory(Base):
    """Wayfair 库存"""
    __tablename__ = "wayfair_inventory"
    ...
    safety_quantity = Column(Integer, default=0)
    ...
    __table_args__ = (Index("ix_wf_inv_sku_warehouse", "sku", "warehouse_id"),)


class WayfairShipment(Base):
    """Wayfair 发货/物流"""
    __tablename__ = "wayfair_shipments"

    id = Column(Integer, primary_key=True, index=True)
    po_number = Column(String(100), index=True, nullable=False)
    wsa_id = Column(String(100), index=True)
    status = Column(String(50), default="pending")
    carrier = Column(String(100))
...
class WayfairCatalogProduct(Base):
    """Wayfair 商品目录缓存"""
    __tablename__ = "wayfair_catalog_products"
    ...
    retail_price_currency = Column(String(10), default="USD")
    raw_data = Column(JSON, default=dict)
    last_synced_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=lambda: datetime.now(ZoneInfo("Asia/Shanghai")))
    updated_at = Column(DateTime, default=lambda: datetime.now(ZoneInfo("Asia/Shanghai")), onupdate=lambda ctx: datetime.now(ZoneInfo("Asia/Shanghai")))

    __table_args__ = (Index("ix_wf_cat_sku", "sku"), Index("ix_wf_cat_product_id", "product_id"))


class WayfairSyncLog(Base):
    """Wayfair 同步日志"""
    __tablename__ = "wayfair_sync_logs"
