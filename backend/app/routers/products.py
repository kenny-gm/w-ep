...
def get_product_cpm_recommendations(
    ...
    ).order_by(AdRecord.record_date.desc()).all()
    ...
    result = []
    for r in records:
        # 点击数存在 clicks 字段，映射到 visitors
        visitors = r.visitors or 0
        if visitors == 0 and hasattr(r, 'clicks') and r.clicks:
            visitors = r.clicks
        result.append({
            "advert_id": r.advert_id or 0,
            "record_date": r.record_date.strftime("%Y-%m-%d") if r.record_date else None,
            "platform": r.platform or "web",
            "payment_type": r.payment_type or "cpm",
            "placements": r.placements or "recommendations",
            "impressions": r.impressions or 0,
            "visitors": visitors,
            ...
            "cpm": round((r.cost or 0) / (r.impressions or 1) * 1000, 2) if r.impressions else 0,
            "cpc": round((r.cost or 0) / visitors, 2) if visitors else 0
        })
...
def get_product_cpm_search(
    ...
    for r in records:
        ...
        result.append({
            ...
            "platform": r.platform or "web",
            "payment_type": r.payment_type or "cpm",
            "placements": r.placements or "search",
            "impressions": r.impressions or 0,
            "visitors": visitors,
            "cost": float(r.cost or 0),
            "order_count": r.order_count or 0,
...
def get_product_cpc_search(
    ...
    # 如果策略1无结果，使用策略2（查询所有有广告ID的非空记录）
    if not records:
        records = db.query(AdRecord).filter(
            AdRecord.product_id == product_id,
            AdRecord.advert_id != 0,
            AdRecord.record_date >= date_from,
            AdRecord.record_date < f"{date_to} 23:59:59"
        ).order_by(AdRecord.record_date.desc()).all()
    ...
    result = []
    for r in records:
        # CPC广告的点击数存在 clicks 字段，映射到 visitors
        visitors = r.visitors or 0
        if visitors == 0 and hasattr(r, 'clicks') and r.clicks:
            visitors = r.clicks
        result.append({
            ...
            "record_date": r.record_date.strftime("%Y-%m-%d") if r.record_date else None,
            "platform": r.platform or "web",
            "payment_type": r.payment_type or "cpc",
            "placements": r.placements or "search",
            "impressions": r.impressions or 0,
            "visitors": visitors,
            "cost": float(r.cost or 0),
