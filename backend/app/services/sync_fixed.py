...
class SyncService:
    ...
    def sync_products(self, limit: int = 100, overwrite: bool = False) -> dict:
        ...
        Args:
            limit: 每批获取数量（Content API 最大支持 100）
    ...
    def sync_ads(self, days: Optional[int] = 30) -> dict:
        ...
        try:
            ...
            # Step 0: 获取广告配置信息 (payment_type, placements)
            adverts = self.client.get_adverts()
            advert_info_map = {}  # advert_id -> {payment_type, placements, nm_ids}
            for ad in adverts:
                ad_id = ad.get("id")
                if not ad_id:
                    continue
            ...
            # Step 1: aggregate by (advert_id, nm_id, record_date, platform/appType)
            # 保留 appType 维度：web(1), android(32), ios(64)
            # 这样 CPM 推荐广告可以按平台查看，CPC 广告也能看到平台细分
            aggregated = {}
            ...
            for stat in stats:
                ...
                for day_data in days_data:
                    ...
                    for app in apps:
                        ...
                        platform = {1: "web", 32: "android", 64: "ios"}.get(app_type, "web")
                        ...
                        for nm_data in nms:
                            ...
                            # aggregate by (advert_id, nm_id, record_date) - 不按 platform 分开
            ...
            # Step 2: write aggregated data to DB (只存每条广告的汇总, platform='')
            count = 0
            updated = 0
            ...
            for (advert_id, nm_id, record_date), data in aggregated.items():
                ...
                # visitors 字段：对于 CPC 广告等于 clicks（fullstats 的 clicks 就是广告点击/访客）
                visitors = data["clicks"]  # fullstats 只返回 clicks，visitors 等于 clicks（对所有广告类型适用）
                ...
                if existing:
                    ...
                    ad_record = AdRecord(
                        ...
                        cart_count=data.get("atbs", 0)  # cart_count = atbs
                    )
        ...
        try:
            ...
            for nm_id_str, daily_data in product_sales_data.items():
                ...
                for date_str, stats in daily_data.items():
                    ...
                    # 查找或创建记录
                    # Delete all existing records with same key to avoid duplicates
                    existing = self.db.query(AdRecord).filter(
                        AdRecord.record_date == record_date,
                    ...
                    # 汇率转换：WB API已按店铺货币返回，CNY店铺直接存储CNY值
                    order_sum = stats.get("order_sum", 0)
                    if False:  # CNY店铺API已返回CNY，无需转换
                        order_sum = order_sum * self.shop.exchange_rate if self.shop.exchange_rate else order_sum
                    if existing:
                        existing.impressions = 0  # product_analytics无广告曝光
                        ...
                        ad_record = AdRecord(
                            ...
                            impressions=0,  # product_analytics无广告曝光
    ...
    def sync_all(self, history: bool = False) -> dict:
        ...
        # 5. 同步产品销售漏斗
        logger.info("开始同步产品销售漏斗...")
        results["product_sales"] = self.sync_product_sales(days=30 if history else 7)
        
        # 6. 同步关键词统计
        logger.info("开始同步关键词统计...")
        results["keywords"] = self.sync_keywords(days=30 if history else 7)
        
        return results
    
    def sync_keywords(self, days: Optional[int] = 7) -> dict:
        """同步关键词统计数据（delegates to sync.py implementation）"""
        # 复用 sync.py 的 sync_keyword_stats 逻辑
