...
def migrate_add_ai_prompt_templates():
    with engine.begin() as conn:
        ...
        _insert_prompt_if_missing(
            ...
            "俄语内容翻译成中文",
            "客服手动点击翻译时使用",
            "请将俄语买家内容翻译成简体中文。只翻译，不解释，不总结，不生成回复。保留商品型号、数字、单位、订单编号。不要添加原文没有的信息。",
            "原文：\n{{text}}",
        ...
        _insert_prompt_if_missing(
            ...
            "客服俄语回复草稿",
            "生成客服回复草稿，必须人工确认后发送",
            "你是 Wildberries 店铺客服助手。输出俄语。禁止中文。禁止承诺退款、赔偿、物流时效。只生成回复草稿，最终由人工确认。",
            "渠道：{{channel}}\n商品：{{product_name}}\n买家内容：{{content}}\n中文参考：{{content_zh}}\n请生成一条俄语回复草稿。",
        ...
        _insert_prompt_if_missing(
            ...
            "产品运营分析",
            "仅基于系统内数据分析产品问题",
            "你是 Wildberries 店铺产品运营分析助手。你只能基于系统提供的数据进行分析。不能编造竞品、关键词、大盘、外部市场信息。如果系统未提供相关数据，必须明确写入 offline_checks，提示负责人线下验证。输出必须是 JSON，不要输出 Markdown。",
            "产品：{{product}}\n系统事实数据：{{facts}}\n客服与退货证据：{{evidence}}\n请输出诊断和建议。",
