<template>
  <div class="ad-analysis minimal-white">
    <!-- 筛选栏 -->
    <FilterBar 
...
function generateMockDailyData(dateFrom) {
  ...
  trafficSource.value = [
    { name: '搜索', value: Math.floor(Math.random() * 5000) },
    { name: '推荐', value: Math.floor(Math.random() * 3000) },
    { name: '类目', value: Math.floor(Math.random() * 2000) },
    { name: '其他', value: Math.floor(Math.random() * 1000) }
  ]
  adCampaigns.value = [
